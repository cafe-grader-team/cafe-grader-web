...
=== Install necessary package
    ...
    cafe@grader:~$ sudo apt install libmysqlclient-dev default-jdk unzip nodejs php7.2-cli apache2 dirmngr gnupg apache2-dev
...
After installation is finished, grader is ready to run in development mode via WEBrick. We will try that by the following command which will start a grader accessible via http://localhost:3000/. You can try logging in to the system and verify that it is actually work.
...
1. Installing Passenger as a mod for apache via PPA and enable it.  
...
2. Deploying the app. First, we determine the exact location of our ruby that is installed via RVM. 
